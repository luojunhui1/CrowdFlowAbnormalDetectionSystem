import { computed } from "vue";
import { useStore } from "vuex";
import { useRouter } from "vue-router";

export default function() {
  const store = useStore();
  const router = useRouter();

  let abcode = computed(
    () => store.state.mapInfo[store.state.mapInfo.length - 1].code
  );
  let year = computed(() => store.state.year);
  let parentInfo = computed(() => store.state.mapInfo);
  let areaIndex = computed(() => store.state.areaIndex);
  let meunIndex = computed(() => store.state.meunIndex);
  const sum = computed(() => store.state.sum);

  const setCommitYear = (val) => {
    store.commit("SET_YEAR", val);
  };
  const setCommitAreaIndex = (val) => {
    store.commit("SET_AREA_INDEX", val);
  };
  const setCommitMeunIndex = (val) => {
    store.commit("SET_MEUN_INDEX", val);
  };
  const setCommitMapInfo = (val) => {
    store.commit("ADD_INFO", val);
  };
  const removeCommitMapInfo = (val) => {
    store.commit("REMOVE_INFO", val);
  };
  const setCommitSum = (val) => {
    store.commit("SET_SUM", val);
  };
  const routerChange = (val) => {
    router.push(val);
  };

  return {
    abcode,
    sum,
    year,
    areaIndex,
    meunIndex,
    parentInfo,
    setCommitYear,
    setCommitSum,
    setCommitAreaIndex,
    setCommitMeunIndex,
    removeCommitMapInfo,
    setCommitMapInfo,
    routerChange,
  };
}
